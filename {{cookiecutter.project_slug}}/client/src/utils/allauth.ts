...
export async function login(data) {
  // Ensure data is properly structured for the login request
  const loginData = {
    email: data.email,
  ...
  // Make the request
  const response = await request("POST", URLs.LOGIN, loginData);

  if (response.status === 409) {
    console.error("Login validation error:", response.errors);
  }
...
export async function requestEmailVerification(email) {
  try {
    // First try the standard method - sending to EMAIL endpoint with action
    const response = await request("POST", URLs.EMAIL, {
      email,
    ...
    // If that fails, try the alternative method - verify_email endpoint directly
    console.log("Falling back to alternative email verification method");
    return await request("POST", "/auth/email/verification/resend", { email });
  } catch (error) {
    ...
    return {
      status: 500,
      errors: [
        { param: "email", message: "Failed to request email verification" },
      ],
    };
  }
}
